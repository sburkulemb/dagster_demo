{{
	config(
		alias = "stg_klaviyo__apip_profiles"
	)
}}

WITH stg_klaviyo__apip_profiles AS
(
SELECT ID , 
...
CASE
		...
		'klaviyo_apip' AS SOURCE_RELATION
	FROM {{ source("raw_klaviyo__apip", "profiles") }}

)

SELECT * FROM stg_klaviyo__apip_profiles
