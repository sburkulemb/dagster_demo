...
WITH first_last_ord_date AS (
	...
	WHERE SOURCE_RELATION = 'shopify_apip'
	GROUP BY EMAIL
	),
...
country_data AS (
	SELECT email,
		...
		'Us', 'U.S. Virgin Islands', 'Us ', 'U.S.', 'United Stated', 'Unites States of America', 'Unitedbstates', 
		'Unites states of america', 'United Statses', 'Untied states', 'Unites states of America ', 'Unites states', 
		'UnitedStates', 'Untited States', 'US', 'United States of America ', 'United states ', 'United state', 'UNITED STATES ',
		'United States if America', 'U.S.A', 'United start ', 'Unites states ', 'Usa', 'USA', 'United stayed ', 'United States or America', 
		'U.S.A. ', 'Unites States of Americs', 'United States 🇺🇸', 'United States ', 'United states', 'United state ', 'Unite states', 'U.S', 
		'UNITED STATES', 'Unites States', 'United states of america', 'United state of America ', 'UNITED STATES OF AMERICA', 'United States of america', 
		'Usa ', 'United State', 'Unite states ', 'United Stares', 'United Stated ', 'United States', 'United stated ', 'US ', 'United State of America', 
		'United states of America ', 'United States Minor Outlying Islands', 'United stated Le ', 'United States (USD $)', 'USA ', 'United States of America',
		'U.S.A.', 'Unites States ', 'Unitedly States of America', 'United sates ', 'United States j', 'U.s', 'Untied states of America ',
		'United States Sarasota county','United stated'
		) THEN 'United States'
		ELSE country  
		END AS country 
	FROM {{ref("int_klaviyo__apip_profiles")}}
	),
...
Orders_data AS(
	...
	AND SOURCE_RELATION = 'shopify_apip'
	),
...
reducing_white_space AS (
	SELECT
		...
		END AS FORMATTED_ZIP_NINE,		
		A.* EXCLUDE(FIRST_NAME,LAST_NAME,ADDRESS_1,ADDRESS_2,CITY,FORMATTED_ZIP_NINE)
...
removing_special_name AS (
	SELECT
		...
		a.* EXCLUDE (FIRST_NAME,LAST_NAME)
